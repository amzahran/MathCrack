        ...
        <div class="minimal-card-wrapper">
            <div class="card mb-4 mt-5 mx-4 mx-sm-0">
                <div class="card-body p-sm-5" style="padding-top: 15px !important;">
                    ...
                    <h2 class="fs-20 fw-bolder mb-4">Reset by SMS OTP</h2>

                    @if(session('success'))
                        <div class="mb-3" style="color: #16a34a;">{{ session('success') }}</div>
                    @endif

                    <form action="{{ route('password.sms.send') }}" method="POST" class="w-100 mb-4">
                        @csrf
                        <input type="hidden" name="email" value="{{ $email ?? old('email') }}">
                        <button type="submit" class="btn btn-light w-100">Send OTP</button>
                    </form>

                    <form action="{{ route('password.sms.verify') }}" method="POST" class="w-100">
                        @csrf
                        <input type="hidden" name="email" value="{{ $email ?? old('email') }}">
                        ...
                        <div class="mt-4 text-muted">
                            <a href="{{ route('password.forgot') }}" class="fw-bold">Change method</a>
                        </div>
                    </form>
