...
@section('css')
    <style>
        /* تكبير عناوين الحقول في صفحة التعديل */
    ...
    <div class="main-content">
        ...
        @can('edit lectures')
            <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                ...
                <div>
                    <a href="{{ route('dashboard.admins.tests-show', ['id' => encrypt($test->id)]) }}" class="btn btn-info waves-effect waves-light mb-2">
            ...
            @php
                ...
                // استنتاج عدد الموديولات من عدد الأسئلة في كل جزء
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $test->name) }}" required>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
...
@section('js')
    <script>
        $(document).ready(function() {
            const hasStudents = {{ $hasStudents ? 'true' : 'false' }};

            function getInt(selector) {
                return parseInt($(selector).val()) || 0;
            }

            // حساب الدرجات (نفس الإنشاء لكن هنا لا نغير total_score لو في طلاب أخذوا الاختبار)
    ...
    // لو مفيش طلاب -> حدّث total_score أوتوماتيك
    if (!hasStudents) {
        ...
        // في طلاب: لا نغيّر total_score، فقط نوضح الفرق
...
updateScoreCalculation();
            ...
            // إظهار / إخفاء الموديولات + البريك حسب modules_count
            function toggleModulesEdit() {
                ...
                $('.module-block').each(function () {
                    ...
                    if (mod <= count) {
                        ...
                        $(this).hide();
                        // نخلي قيم المخفي = 0 (لن يتم تغيير الداتا في الداتابيز إلا لو حفظ)
                        const input = $(this).find('input[type="number"]');
                        if (!{{ $hasStudents ? 'true' : 'false' }}) {
                            input.val(0);
                        }
                    }
                });
            ...
            // قبل الحفظ نخلي أي موديول مخفي بـ 0 (لو مفيش طلاب)
